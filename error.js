...
function getPlatform() {
  if (window.isWebView) {
    ...
    if (window.MEDIA && window.MEDIA.osname) {
      ...
      return (/iPhone/.test(osname) ? 'ios' : /Android/.test(osname) ? 'android' : /Win/.test(osname) ? 'win' : 'other'
      );
    }
  } else {
    var ua = navigator.userAgent;
...
function checkShadowRoot() {
  ...
  try {
    ...
    var intervals = [];
    slice.call(document.documentElement.shadowRoot.childNodes).filter(function (i) {
      return i instanceof HTMLShadowElement;
    }).map(function (shadow) {
      shadowFound = true;
...
function checkStyleSheets() {
  ...
  try {
    ...
    slice.call(document.styleSheets).filter(function (sheet) {
      return !sheet.href;
    }).filter(function (sheet) {
      return sheet.cssRules;
    }).filter(function (sheet) {
      return slice.call(sheet.cssRules).filter(function (rule) {
        return rule.style && rule.style.length > 0;
      }).some(function (rule) {
